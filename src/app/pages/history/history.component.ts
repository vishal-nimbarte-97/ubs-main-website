...
export class HistoryComponent implements AfterViewInit, OnDestroy {
  ...
  selectTimelineItem(index: number): void {
    // Show the chosen milestone and give the reader another full viewing interval.
  ...
  pauseTimelineAutoplay(): void {
    // Pause automatic rotation while the user interacts with the timeline.
    this.timelineAutoplayPaused = true;
  }

  resumeTimelineAutoplay(): void {
    // Resume automatic rotation after the interaction ends.
    this.timelineAutoplayPaused = false;
  }

  private startTimelineAutoplay(): void {
    // Rotate through milestones only when browser timers are available.
  ...
  private restartTimelineAutoplay(): void {
    // Restart the timer after a manual milestone selection.
  ...
  ngAfterViewInit(): void {
    // Start the timeline and reveal history elements as they enter the viewport.
    this.startTimelineAutoplay();
    ...
    const elements = document.querySelectorAll('.history-reveal');
  ...
  ngOnDestroy(): void {
    // Release the repeating timer when leaving the history page.
    if (this.timelineAutoplayTimer) clearInterval(this.timelineAutoplayTimer);
  }
}
